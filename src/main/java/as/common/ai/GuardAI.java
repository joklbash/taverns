 ...
 * <li>ChunkCoordinates position - the position to guard; the entity's current position if <i>null</i>
...
public class GuardAI extends EntityAIBase implements StorableAI
{
	...
	public static GuardAI createFromNBT(EntityLiving living, NBTTagCompound data)
	{
		...
		GuardAI ai = new GuardAI(living, new ChunkCoordinates(posX, posY, posZ),
	...
	public GuardAI(EntityLiving owner, ChunkCoordinates position,
			...
			boolean leaveTransports)
	{
		...
		if( null == position )
		{
			pos = new ChunkCoordinates(MathHelper.floor_double(entity.posX + 0.5),
	...
	{
		data.setInteger("PosX", pos.posX);
		data.setInteger("PosY", pos.posY);
		data.setInteger("PosZ", pos.posZ);
	...
	public boolean shouldExecute()
	{
		...
		double distanceToGuardPointSq = entity.getDistanceSq((double)pos.posX + 0.5, (double)pos.posY, (double)pos.posZ + 0.5);
	...
	public void startExecuting()
	{
		...
		entity.getNavigator().tryMoveToXYZ((double)pos.posX + 0.5, (double)pos.posY, (double)pos.posZ + 0.5, speed);
	}
		...
		switch(state)
		{
			case RUNNING:
				if( entity.getNavigator().noPath()
						|| entity.getDistanceSq((double)pos.posX + 0.5, (double)pos.posY, (double)pos.posZ + 0.5) < roamDistSq / 2 )
				{
